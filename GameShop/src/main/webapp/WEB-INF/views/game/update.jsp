				...
				<div>
					<ul id="files">
						...
						<c:forEach var="attach" items="${item.attachs}">
							<li class="mb-2"><a href="/upload/${attach.filename}">${attach.filename}</a> <button type="button" class="btn btn-sm btn-danger delete" data-id="${attach.id}">삭제</button></li>				
						</c:forEach>
					</ul>
				</div>
				
				<div class="form-group">
					<label>첨부파일: <button type="button" id="add" class="btn btn-sm btn-primary mt-2 mb-2">추가</button></label>
					<div id="attachs">
